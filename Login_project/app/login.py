...
def forget_password():
    ...
    def reset_password():
        if securityquescombo.get() == 'Select' or answerforgetEntry.get() == '' or newpassEntry.get() == '':
            ...
            try:
                ...
                cur = con.cursor()
                cur.execute('select * from students where email=%s and question=%s and answer=%s',
    ...
    if mailentry.get() == '':
        ...
        try:
            con = pymysql.connect(host='localhost', user='root', password='@rotuser', database='register')
            cur = con.cursor()
...
def signin():
    if mailentry.get() == '' or passentry.get() == '':
        ...
        try:
            con = pymysql.connect(host='localhost', user='root', password='@rootuser', database='register')
            cur = con.cursor()
            cur.execute('select * from students where email=%s and password=%s', (mailentry.get(), passentry.get()))
            row = cur.fetchone()
            if row == None:
